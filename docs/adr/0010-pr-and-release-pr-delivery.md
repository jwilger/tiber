...
GitHub Release and publish through npm trusted OIDC with provenance. The pinned
release-please job invokes a separate least-privilege reusable publication
workflow only from its successful exact release outputs. Do not rely on a
GitHub Release event created by the workflow token to start publication,
because GitHub intentionally suppresses recursive workflow triggers for such
events. Permit explicit exact-tag dispatch only for audited recovery.
