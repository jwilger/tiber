//! Read-only native Tiber Tasks command adapter.
//!
...
/// Executes one read-only `tiber tasks` invocation against the current repository.
pub(super) fn run(
    repository: &Path,
    ...
    match command {
        TaskCommand::List(status) => Ok(list_tasks(&projection, status)),
        TaskCommand::Show(reference) => show_task(&projection, &reference),
        TaskCommand::Search(query) => Ok(search_tasks(&projection, &query)),
        TaskCommand::Next => next_task(&projection),
    }
}
